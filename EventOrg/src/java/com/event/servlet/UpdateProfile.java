...
public class UpdateProfile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            int update = userdao.updateUserDetails(uid, name, email, number, branch);

            // update iterrests
            String arr = request.getParameter("intArr");


            // delete previous interests
            UserInterestMapDao uidao = new UserInterestMapDao(ConnectionProvider.getConnection());

            int delete = uidao.deleteUserInterestsByUserId(uid);
         

            // adding new interests
            InterestDao dao = new InterestDao(ConnectionProvider.getConnection());

            String interests[] = arr.split(",");
            ...
            for (int i = 0; i < m; i++) {
                ...
                if (id != -1) {
                    ...
                    boolean f = uidao.saveUserInterestMap(uimap);
